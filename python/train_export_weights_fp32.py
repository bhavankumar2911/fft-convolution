...
# --------------------------------------------------
# Save model weights as raw float32 .bin files
# --------------------------------------------------
def save_model_weights_as_bin_float32(
    model: nn.Module,
    output_dir: str
):
    ...
    for name, param in model.state_dict().items():
        ...
        assert param_numpy.dtype == np.float32
        ...
        metadata_lines.append(
            f"{filename} shape={param_numpy.shape} dtype=float32"
        )
...
def main():
    ...
    # -----------------------------
    # Export FP32 weights (CPU)
    # -----------------------------
    model_cpu = model.to("cpu")
    model_cpu.eval()

    save_model_weights_as_bin_float32(
        model=model_cpu,
        output_dir=weights_bin_dir
    )

    print(f"Saved FP32 weights to: {weights_bin_dir}")
